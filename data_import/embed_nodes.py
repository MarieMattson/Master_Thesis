...
def process_anforande(item):
    ...
    remaining = count_remaining_anforande_without_chunks()
    print(f"Remaining anföranden without chunks: {remaining}")
...
def count_remaining_anforande_without_chunks():
    with driver.session() as session:
        result = session.run("""
            MATCH (a:Anforande)
            WHERE NOT (a)-[:HAS_CHUNK]->()
...
if not data:
    ...
    print(f"Found {len(data)} total anföranden.")
    
    max_workers = 1  # Adjust this number depending on your system & Neo4j capacity
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        futures = [executor.submit(process_anforande, item) for item in data]

        for future in as_completed(futures):
            try:
                future.result()  # If any exceptions happen inside the thread, this will raise them here
            except Exception as e:
                print(f"Error processing anforande: {e}")
